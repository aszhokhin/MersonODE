*  THE GENERAL SOLUTION
      ...
      DO1 I=1,LD
    1 X(I)=X0(I)
      CALL SOLEQ(LD,KD,X,U,T1,T,D3EQ)
      ...
      SUBROUTINE OPN6
      IMPLICIT REAL*8 (A-H,O-Z)
      OPEN(6,FILE='D3EQ.RES',STATUS='UNKNOWN')
      RETURN
      END
      ...
      IMPLICIT REAL*8 (A-H,O-Z)
      REAL*8 U(M,N)
      OPEN(3,FILE='D3EQ.DAT',STATUS='UNKNOWN')
      DO1 I=1,M
      TI=(I-1)*(T-T1)/(M-1)+T1
    1 WRITE(3,2)TI,U(I,1),U(I,2),U(I,3)
    2 FORMAT(5E15.5)
      RETURN
      END
      ...
      COMMON/PRM3/P
      F(1)=-.2*X(1)+(X(3)-1)*X(2)
      F(2)=-1.08*X(2)+(X(3)+1)*X(1)
      F(3)=-.1*X(3)-2*X(1)*X(2)+5
      RETURN
      END
